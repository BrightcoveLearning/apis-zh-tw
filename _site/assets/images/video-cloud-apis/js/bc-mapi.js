...
var BCMAPI = new function () {
	...
	this.calls = [
		...
		{ "command" : "search_videos", "def" : false },
		{ "command" : "search_videos_unfiltered", "def" : false}
	];
			...
			if ((typeof pParams == "object") && pParams) {
				for (var pParam in pParams) {
					if (pParam == "any" || pParam == "all" || pParam == "none") {
						...
						} else if (pParams[pParam].indexOf("," > -1)) {
							var pArray = pParams[pParam].split(",");
							for (var idx in pArray) {
								pQuery += "&" + pParam + "=" + encodeURIComponent(pArray[idx]);
							}
						} else {
							pQuery += "&" + pParam + "=" + encodeURIComponent(pParams[pParam]);
						}
					} else if (pParam == "selector") {
